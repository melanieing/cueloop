...
6. 일일 목표 + 스트릭 + 백업/복원
7. 전역 ON/OFF 토글 (영상 컨트롤바 `🎬 Cueloop` 버튼 — 끄면 오버레이·단축키 정지 → 그냥 넷플릭스, 가벼운 시청용. 컨트롤바와 idle 동기화, 좁은 화면 컴팩트. 상태는 `chrome.storage.local`)
8. 자막 표시 순서 선택 (영어 위 / 한국어 위, 옵션에서 전환 — `chrome.storage.local`)
