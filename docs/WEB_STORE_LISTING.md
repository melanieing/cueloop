...
🧹 라인·콘텐츠 정리
라인 호버 시 한 줄 삭제, "여러 줄 삭제" 모드로 다중 선택 후 일괄 삭제. 영화 자체도 헤더 휴지통 버튼으로 자막·진도·CustomLoop까지 한 번에 정리. 시청목록 빠진 영화도 깨끗하게.
...
🎓 첫 설치 시 사용법 안내
확장을 처음 설치하면 옵션 페이지가 자동으로 열려서 8단계 시작하기 가이드를 보여줍니다. 단축키 cheat sheet도 함께.
...
- ☑ Memorized: mark lines you no longer need; hide them to focus on the rest
- Header toggles filter each independently or combine them all

🧹 Line & Content Cleanup
Hover a line for a single-line delete; the "여러 줄 삭제 / Multi-select" mode lets you select many and bulk-delete in one Dexie transaction. Whole movies can be wiped (subtitles + progress + CustomLoops together) from the header trash button — useful for cleaning out titles you no longer watch.
...
On first install the options page opens automatically with an 8-step "Getting Started" guide and the keyboard cheat sheet, so you never have to wonder how to begin.
