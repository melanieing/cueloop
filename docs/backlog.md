...
- **단계**: v0.2 / **Web Store Phase 1 운영 중**. v0.2.0 게시(2026-05-30) → **v0.2.2 게시 완료** → 자막 공유 기능 추가해 **v0.2.3 제출(2026-06-04), 심사 대기**. 통과 시 기존 사용자 자동 업데이트.
- **마지막 업데이트**: 2026-06-04
...
## v0.2.2 — hide 기능 포함해 재제출 → ✅ 게시 통과
v0.2.1을 심사 중 취소하고 아래 hide 기능을 더해 0.2.2로 bump → 단일 심사로 묶어 제출 → 통과·게시. 권한 변경 없음.
...
- [ ] **v0.2.3 심사 통과 대기** — 통과 후에야 블로그 독자가 import 가능. 블로그 자막 공유 섹션은 이미 게시함(velog).

> v0.2.2가 게시 통과한 상태라 0.2.3은 그 위에 얹는 일반 업데이트로 제출.

## v0.2.4 누적 (v0.2.3 제출 이후 git main에 쌓이는 변경 — v0.2.3 통과 후 다음 업데이트로 묶음)
- [x] 라인 복사(분할용) — 라인 hover 시 복사 버튼(파란 DuplicateIcon). 바로 아래에 새 라인 생성: 시작=원본 종료, 종료=원본 종료+1분(분할 중 끊을 여유), 영/한/메모 복사, source='user'. 마크(외움/중요/검토/숨김)·반복횟수는 복사 안 함.
- [x] InsertLineModal(새 라인 추가) 메모 input → textarea (EditRow와 통일)
- [ ] **v0.2.4 제출 대기** — v0.2.3 통과 후 bump + zip + 업로드
