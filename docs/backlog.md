...
## v0.2.2 누적 (v0.2.1 제출 이후 git main에 쌓이는 변경 — 다음 업데이트로 묶음)
- 라인 숨김(hide) 기능 — 노래 가사 등 학습 대상 아닌 라인을 삭제 대신 목록에서만 숨김 (`Line.isHidden`). 라인별 eye-off 토글 + `🙈 숨김 N` 필터 + 일괄 숨김(선택 모드 액션)
- "여러 줄 삭제" 버튼 → "☑ 여러 줄 선택"으로 변경 + 선택 모드에 `🙈 선택 숨김` 추가, 선택 색상 중립화
- 통계 라인 재구성 — 한국어 % 제거, 학습대상 라인 수(전체−숨김) + ☑외움 비율(외움/(전체−숨김)) 표시
